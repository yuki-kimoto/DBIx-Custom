...
You can also pass it to C<where> of C<update()>、C<delete()>
...
If you need table name、you can get it by C<table()>.
...
If C<table> is 'book'、C<column> is ['id', 'name'],
...
If C<table> is 'company'、C<column> is ['id', 'name'],
the following clause is created.
